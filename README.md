...
Life Coach 是一套公开、长期维护的 Codex skills，陪你把人生方向转化为目标、项目和今日行动，同时照看精力、拖延、习惯与复盘。

你决定去哪里，小芽负责帮助看清路径、排序约束、推进下一步，并让经验沉淀下来。

配套有记忆规则让 AI 在对话中逐步记录你的作息习惯、精力窗口、常见拖延原因、有效启动方式、项目状态等。小芽不只是回答“今天怎么安排”，而是逐渐知道什么计划对你现实、什么提醒会反效果、哪些目标真正靠近你的阶段主题。

整个项目流程涉及 7 个模块：
`人生愿景 -> 目标设定 -> 项目拆解 -> 精力与日程 -> 任务与习惯 -> 复杂问题 -> 检验复盘`

每个模块都配置了对应的方法工具、认知概念和执行原则。当前公开版本包含 7 条主工作流、49 个流程阶段、68 个方法工具章节和 76 个认知/行为概念章节，合计约 140 个可复用的方法与概念条目。

理论来源横跨 ACT 接纳承诺疗法、CBT 认知行为疗法、正念与自我关怀、ICF 教练能力、行为设计、人生设计、存在主义心理治疗、复杂问题解决和 PDCA 工作循环等方向。它把这些公开理论与实践方法重新组织成一套可由 Codex 调度、可长期记录、可持续迭代的人生教练工作流。

![小芽人生教练系统流程图](_image/小芽人生教练系统流程图.png)

## 结构介绍

当前版本中，`AGENTS.md` 明确总调度人格和路由规则，可复用能力以独立 skill 的形式放在 `skills/` 下。
...
把这个仓库作为 Codex skill pack 使用时，保留 `AGENTS.md`、`references/` 和 `skills/` 目录结构。

- `AGENTS.md` 是小芽的人生教练总调度说明，负责角色、路由、沟通风格和安全边界。
- `references/coaching-process.md` 是七个 skill 共用的教练与聊愈流程，负责安全建立、倾听、澄清、觉察、行动和收尾。
- `references/act-core.md` 是 ACT 底层操作参考，适用于自我评判、情绪淹没、回避、价值冲突、选择困难和“想清楚但行动不了”的场景。
...
效率工具可以提供优先级标签，但是无法替我选择优先级。

作者当前是通过notion + 滴答清单的组合来完成每日安排的，小芽则通过MCP获取和修改日程。Notion负责记录愿景、OKR、项目计划、项目复盘等内容，滴答清单负责具体的任务打卡。后文中有更详细的工具推荐。

你只需要选择自己习惯的工具接入即可，大部分工具应该都开放了MCP或API供AI使用。

### skill中仅保存了体系的核心概念

AI能把当前这些理论融会贯通到什么程度，多少内容能被有效阅读是一个等待检验的问题。后续将根据反馈，持续调整和补充说明。

### 画像记忆不是数据堆砌得越多越好

相比于记录的数量，记录的维度更加重要。垃圾的问题只能带来无效的积累。

目前的记忆规则是AI根据理论自行设计的，因为我原有的记录系统依托复杂的notion形式，并且高度个人化，不适合迁移。

你在使用的过程中，也可以让AI帮你设计适合自己的记录形式。
...
Life Coach 可以只作为一组 skills 使用，也可以加上一层记忆系统，让它通过可检查、可修改的记录，逐渐更懂你的真实生活：你的作息、固定承诺、精力窗口、常见拖延原因、有效启动方式、项目状态、计划偏离和复盘洞察。

这意味着，小芽不会每次都从零开始问“你什么时候有空”“你为什么拖延”“你适合什么计划”。在长期使用中，它可以基于记录逐步校准：哪些时间适合高认知任务，哪些安排总是过载，什么样的提醒会让你反感，什么样的最小行动能帮你启动，哪些目标真的靠近你的阶段主题。

默认推荐先用被 `.gitignore` 排除的 `life-coach-data/` 保存个人数据，并从少量核心文件开始：用户画像、人生罗盘、项目清单、周/日计划、习惯追踪、复盘日志和长期记忆。不要一开始拆太多专项文件；等某类记录反复出现、需要频繁单独读取时，再让 AI 帮你拆分。这样不依赖任何外部服务，也方便理解这套 agent 到底需要哪些上下文。

这套记录原则是“先少后多、单一事实源、先记录再提炼”：

- `profile/user.md` 先记录稳定底图，例如作息、固定承诺、沟通偏好、精力线索、执行/拖延线索。
- `profile/life-compass.md` 记录人生 8+1、价值、阶段主题和人生实验。
- `planning/`、`projects/`、`habits/`、`reviews/` 记录计划、项目、习惯和复盘的运行过程。
- `memory/daily/` 记录当天事实和候选洞察，`memory/long-term.md` 只放跨文件综合洞察和索引，不做第二份用户画像。
- 单次观察先进入日记或复盘，多次出现后再沉淀成长期规律。

使用者可以根据自己的习惯 DIY 记忆体系：继续用本地 Markdown，或让 AI 辅助配置外部 MCP/API。例如：
...
- 精力、拖延、复盘等记录可以先放在 `user.md` 和 `review-log.md`，等积累多了再拆成专项文件。
