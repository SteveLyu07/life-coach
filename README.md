...
Life Coach 可以只作为一组 skills 使用，也可以加上一层记忆系统。默认推荐先用被 `.gitignore` 排除的 `life-coach-data/` 保存个人数据，并从少量核心文件开始：用户画像、人生罗盘、项目清单、周/日计划、习惯追踪、复盘日志和长期记忆。不要一开始拆太多专项文件；等某类记录反复出现、需要频繁单独读取时，再让 AI 帮你拆分。这样不依赖任何外部服务，也方便理解这套 agent 到底需要哪些上下文。
